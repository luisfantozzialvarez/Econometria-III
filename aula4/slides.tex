...
\begin{frame}{Teste Dickey-Fuller aumentado}
	...
	\begin{halfwideitemize}
		...
		\begin{halfwideitemize}
			\item No caso geral, $\epsilon_t\sim I(0)$, mas erros podem ser autocorrelacionados. \hyperlink{app_adf}{\beamergotobutton{Detalhes}} 
		\end{halfwideitemize}
...
\begin{frame}{Procedimento Sequencial I}
		...
		{\usebeamercolor[fg]{structure} 1.} Comece estimando o modelo mais geral:
		\begin{equation*}
			\begin{aligned}
				\Delta Y_t = \alpha + \beta \cdot t + \gamma Y_{t-1}  +\sum_{j=1}^k \kappa_j\Delta Y_{t-j} + u_t
			\end{aligned}
		...
		\begin{halfwideenumerate}
			...
			\item Se \textbf{não} rejeitamos a hipótese nula, fazemos o teste F da nula $(\beta, \gamma)=(0,0)$, usando os valores críticos do slide 11 (Caso 4).
			\begin{halfwideenumerate}
\footnotesize
...
\begin{frame}{Procedimento Sequencial II}
		...
		{\usebeamercolor[fg]{structure} 2.} Se chegamos a essa etapa, não temos evidências de que haja uma tendência linear no modelo. Nesse caso, estimamos:
		\begin{equation*}
			\begin{aligned}
				\Delta Y_t = \alpha  + \gamma Y_{t-1}  +\sum_{j=1}^k \kappa_j \Delta Y_{t-j} + u_t
			\end{aligned}
		...
		\begin{halfwideenumerate}
			...
			\item[2.2] Se \textbf{não} rejeitamos a hipótese nula, fazemos o teste F da nula $(\alpha, \gamma)=(0,0)$, usando os valores críticos do slide 11 (Caso 2).
			\begin{halfwideenumerate}
				\footnotesize
...
\begin{frame}{Testando a presença de componentes determinísticos}
	\begin{halfwideitemize}
		...
		\item Se a série não apresenta raiz unitária, trabalhamos com $Z_t = Y_t$.
...
\begin{frame}{Procedimento de \citet{Dickey1987}}
	\begin{itemize}
		...
		\item Nesse ambiente, \citet{Dickey1987} sugerem testar $H_0: \gamma_{1,1} = 0 $ contra $H_1: \gamma_{1,1} < 0$, usando estatística $t$ e valores critícos de Dickey-Fuller.
			\begin{itemize}
				\item Se não rejeitamos a nula, concluímos que a série apresenta DUAS raízes unitárias.
			\end{itemize}
		...
		\vspace{-1em}
		\begin{itemize}
			\item Se não rejeitamos a nula, concluímos que série apresenta UMA raiz unitária.
			\item Se rejeitamos a nula, série NÃO APRESENTA raiz unitária.
		\end{itemize}
...
\begin{frame}{Motivação do teste ADF}
	\footnotesize
	\label{app_adf}
	\begin{itemize}
		\item No caso geral de um ARMA(p,q) sem componentes determinísticos, teste da hipótese nula de uma raiz unitária contra alternativa de nenhuma raiz dentro do círculo, consiste, no modelo:
		
		$$\Phi(L)(1-\phi L)y_t = \Theta(L)v_t\, ,$$
		em que $\Phi(L)$ exibe todas raízes fora do círculo e $v_t$ é ruído branco, em se testar:
		
		$$H_0: \phi = 1$$
		contra a alternativa:
		$$H_1: \phi < 1\, ,$$
		Como $\Phi(L)$ tem todas as raízes foram do círculo, segue que $(1-\phi L)y_t$ é um MA($\infty$), donde obtemos a especificação:
		\begin{equation*}
			\begin{aligned}
				(1-\phi L)y_t =  \sum_{j=1}^\infty \pi_j v_{t-j} \implies 
				\Delta y_t = \underbrace{\gamma}_{=\phi-1} y_{t-1} + \underbrace{v_t + \sum_{j=1}^\infty \pi_j v_{t-j}}_{=\text{erro}_t}
			\end{aligned}
		\end{equation*}
		\item Ideia de \citet{Dickey1987}: se polinômio $\Theta$ apresenta todas as raízes fora do círculo, então dependência em $\sum_{j=1}^\infty \pi_j \epsilon_{t-j}$ é bem aproximável, sob a nula, por termo da forma $\sum_{j=1}^k \rho_j \Delta y_{t-j}$ para algum $k$. \hfill  \hyperlink{main_adf}{\beamerreturnbutton{Retornar à aula}} 
	\end{itemize}
