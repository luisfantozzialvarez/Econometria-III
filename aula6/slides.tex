...
\begin{frame}{Previsão}
	\begin{itemize}
		...
		$$\lim_{h \to \infty }\hat{\boldsymbol{X}}_{T+h} = (\mathbb{I}-  \hat{\boldsymbol{A}}_1 -  \hat{\boldsymbol{A}}_2 \ldots -  \hat{\boldsymbol{A}}_p )^{-1}\hat{\boldsymbol{a} }$$
	\end{itemize}
\end{frame}
...
\begin{frame}{Modelo condicional e exogeneidade fraca}
	\begin{itemize}
		\item Formalmente, um modelo é uma parametrização da distribuição condicional  $f[\boldsymbol{Z}_t|\mathcal{Z}_{t-1}] = f[\boldsymbol{Z}_t|\mathcal{Z}_{t-1};\theta_0] $, onde $\theta_0 \in \Theta$ é um parâmetro desconhecido, que tomar algum valor em $\Theta$.
...
\begin{itemize}
	\item Retorne à faturação da densidade conjunta:
...
\begin{itemize}
	...
	\end{equation*}
	onde $\xi$ são os parâmetros fundamentais (preferências, tecnologia, política fiscal e monetária etc.), e tanto os  choques e $\boldsymbol{A}_l$ são funções complicadas desses parâmetros que emergem do equilíbrio do sistema.
	\item Ideia é estimar $\theta$ indiretamente a partir do VAR(p), e realizar os contrafactuais variando-se $\theta$.
	\begin{itemize}
