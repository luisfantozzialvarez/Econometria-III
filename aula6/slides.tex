...
\begin{frame}{Selecionando a ordem $p$ de um VAR}
	\begin{itemize}
		...
		$$\text{AIC}(p) = \operatorname{ln}(\operatorname{det}(\hat\Sigma(p))) + \frac{2}{T}(d^2 p + d)\, ,$$
						$$\text{HC}(p) = \operatorname{ln}(\operatorname{det}(\hat\Sigma(p))) + \frac{2 \log \log(T)}{T}(d^2 p + d)\, ,$$
	...
	\item Métodos oferecem aproximações ao erro quadrático médio de previsão um passo à frente.
	\end{itemize}
\end{frame}

\begin{frame}{Testando a inclusão de defasagens}
\begin{itemize}
	\item Considerado um VAR(p)
\end{itemize}
\end{frame}
