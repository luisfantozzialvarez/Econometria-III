...
\begin{frame}{Estimação incondicional do ARMA(p,q)}
	\begin{itemize}
		...
		\begin{itemize}
			\item Por exemplo, se $\epsilon_t \overset{\text{iid}}{\sim} N(0,\sigma^2)$ e $Y_t$ seguir um AR(1) estacionário, então $Y_1 \sim N(\alpha/(1-\rho), \sigma^2/(1-\rho^2))$.
		\end{itemize}
		...
		\end{itemize}
		\item Se ruído branco de fato é Gaussiano, estimador é {\color{blue}eficiente}: dentre todos os estimadores consistentes dos parâmetros de um ARMA(p,q) Gaussiano, estimador é o de menor variância.
